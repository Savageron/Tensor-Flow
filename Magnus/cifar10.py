...
def _get_file_path(filename):
    ...
    return os.path.join(data_path, filename) #(data_path, "cifar-10-batches-py/", filename)
...
#            data = pickle.load(file, encoding='bytes') ###latin1, bytes, ASCII did not work 
...
###how about use record_bytes = tf.decode_raw(value, tf.uint8) and use utf-8 instead of uint8
    file_path = _get_file_path(filename)
    print("Loading data: " + file_path)
...
#    raw = _unpickle(filename="batches.meta.txt") ###(filename="batches.meta"[b'label_names'])
...
#    return names
    ...
    with open(data_path + "batches.meta.txt","r") as f:
...
def load_training_data():
    ...
    for i in range(_num_files_train):
        ...
        images_batch, cls_batch = _load_data(filename="data_batch_" + str(i + 1) + ".txt") ###".bin"
...
def load_test_data():
    ...
    images, cls = _load_data(filename="test_batch.txt") ###.bin
